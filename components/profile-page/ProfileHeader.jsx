...
const ProfileHeader = ({ profile, followers, following, user }) => {
  ...
  useEffect(() => {
    document.title = `${profile.user.name}'s Profile on Prblm Solvers`;
  }, [profile]);
          ...
          {user && user._id === profile.user._id ? (
            ...
            isFollowing ? (
              <button
                ...
                className="bg-rose-100 text-rose-600 text flex items-center font-semibold rounded-md px-3 py-2"
              >
