...
const PostPage = ({ user }) => {
  ...
  useEffect(() => {
    document.title = data && `${data.title} on Prblm Solvers`;
  }, [data]);
      ...
      <div className="max-w-5xl px-4 py-8 md:px-12 md:py-12 mx-auto">
        ...
        <div className="flex flex-wrap md:flex-nowrap">
          <div className="w-full md:w-2/3 lg:w-3/4">
            ...
            ></div>
            <div className="mt-6">
              <h1 className="mb-4 text-lg text-rose-600 font-semibold">
            ...
            <div className="grid grid-col-1 mt-4 space-y-2">
              ...
              <div className="flex flex-wrap items-center border-b py-1">
                <div className="w-5 mr-2">
                  <TagIcon className="h-5 w-5 text-rose-600" />
                </div>
                <div className="flex-1 flex flex-wrap gap-2">
                  {data.techStack.map((tag, index) => (
                    <Link key={index} href={`/posts/tag/${tag}`}>
                      <a className="bg-gray-100 hover:bg-rose-600 hover:text-white transition text-gray-800 text-sm font-semibold rounded-md px-2 py-1">
                        {tag}
                      </a>
                    </Link>
                  ))}
