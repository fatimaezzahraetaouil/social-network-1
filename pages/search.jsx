    ...
    <div>
      ...
      >
        ...
        <input
          ...
          className="focus:ring-gray-100 focus:ring-2 focus:border-rose-500 block w-full pl-10 pr-12 py-4 sm:text-lg border-none rounded-lg"
          placeholder="Search..."
          required
        />
        <div className="absolute inset-y-0 right-0 flex items-center">
          ...
          <select
            id="search-for"
            className="focus:ring-gray-100 focus:ring-2 focus:border-rose-500 h-full py-0 pl-2 pr-7 border-transparent bg-transparent text-gray-500 sm:text-md rounded-lg"
            value={type}
      ...
      {data?.pages[0]?.posts?.length === 0 && (
        <p className="text-rose-600 text-center my-8 text-lg">No posts found</p>
      )}
            ...
            <Fragment key={i}>
              {page.users.map((user) => (
                <Link key={user._id} href={`/${user.username}`}>
                  <div className="hover:text-rose-600 transition cursor-pointer">
      ...
      {data?.pages[0]?.users?.length === 0 && (
        <p className="text-rose-600 text-center my-8 text-lg">No users found</p>
      )}
      {(isLoading || isFetching) && (
        <AiOutlineLoading3Quarters className="h-8 my-8 animate-spin mx-auto w-8 text-rose-600" />
      )}
    </div>
  );
};
...
export async function getServerSideProps() {
  return { props: { title: 'Advanced Search on Prblm Solvers' } };
}
